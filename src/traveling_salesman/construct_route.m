...
[row, col] = find(dist == min_dist); % 在dist中找出min_dist对应的行和列
first = row(1); % 将min_dist 在dist中对应的行序号作为起点

unvisited(unvisited == first) = []; % 将first从unvisited中删除
visited = [visited, first]; % 把first添加到visited中
pre_point = first; % 把first赋值给pre_point
while ~isempty(unvisited)
    pre_dist = dist(pre_point, :); % pre_point与其他城市的距离
    pre_dist(visited) = inf; % 将pre_point与已经添加进来的城市之间的距离设置为inf
    [~, pre_point] = min(pre_dist); % 找出pre_point的最小值
    unvisited(unvisited == pre_point) = []; % 将pre_point从unvisited中删除
    visited = [visited, pre_point]; % 把pre_point添加到visited中
end
